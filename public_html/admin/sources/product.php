...
function save_item(){
	...
	$file_name1=$_FILES['file5']['name'];
	$file_name2=$_FILES['file6']['name'];
	$file_name3=$_FILES['file7']['name'];
	...
	if($id){
		...
		if($photo1 = upload_image("file5", _format_duoihinh, _upload_sanpham,$file_name1)){
			$data['photo5'] = $photo1;	
			//$data['thumb'] = create_thumb($data['photo5'], 220, 180, _upload_sanpham,$file_name1,2);									
			$d->setTable('product');
			$d->setWhere('id', $id);
			$d->select();
			if($d->num_rows()>0){
				$row = $d->fetch_array();
				delete_file(_upload_sanpham.$row['photo5']);	
				//delete_file(_upload_sanpham.$row['thumb']);								
			}
		}


		if($photo2 = upload_image("file6", _format_duoihinh, _upload_sanpham,$file_name2)){
			$data['photo6'] = $photo2;	
			//$data['thumb'] = create_thumb($data['photo'], 220, 180, _upload_sanpham,$file_name2,2);									
			$d->setTable('product');
			$d->setWhere('id', $id);
			$d->select();
			if($d->num_rows()>0){
				$row = $d->fetch_array();
				delete_file(_upload_sanpham.$row['photo6']);	
				//delete_file(_upload_sanpham.$row['thumb']);								
			}
		}


		if($photo3 = upload_image("file7", _format_duoihinh, _upload_sanpham,$file_name3)){
			$data['photo7'] = $photo3;	
			//$data['thumb'] = create_thumb($data['photo'], 220, 180, _upload_sanpham,$file_name3,2);									
			$d->setTable('product');
			$d->setWhere('id', $id);
			$d->select();
			if($d->num_rows()>0){
				$row = $d->fetch_array();
				delete_file(_upload_sanpham.$row['photo7']);	
				//delete_file(_upload_sanpham.$row['thumb']);								
			}
		}

		if($download = upload_image("file1", _format_duoitailieu, _upload_sanpham,$file_name)){
		...
		foreach ($config['lang'] as $key => $value) {
			...
			$data['motawear'.$key] = magic_quote($_POST['motawear'.$key]);
			$data['motaprin'.$key] = magic_quote($_POST['motaprin'.$key]);
			$data['motacorr'.$key] = magic_quote($_POST['motacorr'.$key]);
		...
		if($photo = upload_image("file5", _format_duoihinh, _upload_sanpham,$file_name1))
		{
			$data['photo5'] = $photo1;		
			//$data['thumb'] = create_thumb($data['photo'], 220, 180, _upload_sanpham,$file_name,2);	
		}

		if($photo = upload_image("file6", _format_duoihinh, _upload_sanpham,$file_name2))
		{
			$data['photo6'] = $photo2;		
			//$data['thumb'] = create_thumb($data['photo'], 220, 180, _upload_sanpham,$file_name,2);	
		}

		if($photo = upload_image("file7", _format_duoihinh, _upload_sanpham,$file_name3))
		{
			$data['photo7'] = $photo3;		
			//$data['thumb'] = create_thumb($data['photo'], 220, 180, _upload_sanpham,$file_name,2);	
		}
...
function save_danhmuc(){
	...
	if($id){		
		...
		$id =  themdau($_POST['id']);
		if($photo2 = upload_image("file2", _format_duoihinh, _upload_sanpham,$file_name)){
			$data['photo2'] = $photo2;	
			$d->setTable('product_danhmuc');
			$d->setWhere('id', $id);
			$d->select();
...
function delete_photo(){
	...
	if(isset($_GET['id'])){
		$id =  themdau($_GET['id']);
		$d->reset();
		$sql = "select id, download from #_product where id='".$id."'";
		$d->query($sql);
		if($d->num_rows()>0){
			while($row = $d->fetch_array()){
				delete_file(_upload_product.$row['download']);	
			}
		$sql = "UPDATE #_product SET download ='' WHERE  id = '".$id."'";
		$d->query($sql);
		}
		if($d->query($sql))
			redirect("index.php?com=product&act=capnhat&type='".$_REQUEST['type']."'");
		else
			transfer("Xóa dữ liệu bị lỗi", "index.php?com=product&act=capnhat&type=".$_REQUEST['type']."");
	}else transfer("Không nhận được dữ liệu", "index.php?com=product&act=capnhat&type=".$_REQUEST['type']."");
}
?>