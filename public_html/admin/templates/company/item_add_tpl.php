...
<form name="supplier" id="validate" class="form" action="index.php?com=company&act=save" method="post" enctype="multipart/form-data">
    ...
    <div class="title"><img src="./images/icons/dark/users.png" alt="" class="titleIcon" />
       ...
       <?php } ?>
        ...
        <div class="formRow">
          <label>Ngôn ngữ mặc định: <img src="./images/question-button.png" alt="Chọn loại" class="icon_que tipS" original-title="Check chọn ngôn ngữ mặc định cho website"> </label>
          <div class="formRight"> 
           <?php foreach ($config['lang'] as $key => $value) { ?>
				<input type="radio" name="lang" id="lang<?=$key?>" value="<?=$key?>" <?=($item['lang']==$key)?'checked="checked"':''?> />
    ...
    <div class="form-editor content-item-map">
     ...
     <div class="formRow">
        <label>Thời gian chạy slider: </label>
        <div class="formRight">
          <input type="text" value="<?=@$item['time']?>" name="time" title="Thời gian chạy slider" class="tipS" />
           <!--  <input type="text"  name="thoigianchay" value="<?=@$item['time']?>"  title="Thời gian chạy slider" class="tipS" onkeypress="return OnlyNumber(event)" /> -->
        </div>
        <div class="clear"></div>
    </div>  
